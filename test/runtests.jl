...
function test_interp()
    ...
    nodes_coords = [space.objects_per_dimension[1].object[node].geometry for node in nodes]
    ...
    get_electron_temperature = interp(ids.edge_profiles.ggd[1].electrons.temperature[1], kdtree)
    ...
    grid_val = ids.edge_profiles.ggd[1].ion[1].density[1].values[chosen_index]
    chosen_nodes = [space.objects_per_dimension[3].object[ii].nodes for ii in chosen_index]
    cell_centers = [Tuple(mean([space.objects_per_dimension[1].object[node].geometry for node in nodes])) for nodes in chosen_nodes]
...
function test_project_prop_on_subset()
    ...
    from_subset = get_grid_subset_with_index(dd.edge_profiles.grid_ggd[1], 5)   # All cells
